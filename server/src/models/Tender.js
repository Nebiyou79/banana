...
tenderSchema.methods.canUserView = async function (userId, userRole) {
  try {
    ...
    if (this.tenderCategory === 'freelance') {
      // Freelance tenders visible to freelancers and owners
      if (userRole !== 'freelancer' && userRole !== 'admin') {
        // Companies and organizations shouldn't see freelance tenders
        if (userRole === 'company' || userRole === 'organization') {
          return false;
        }
      }

      // Must be published and active for non-owners
