...
// In User.js - Update the portfolio item schema
const portfolioItemSchema = new mongoose.Schema({
  title: { type: String, required: true, trim: true, maxlength: 200 },
  description: { type: String, trim: true, maxlength: 1000 },
  mediaUrl: {  // Keep for backward compatibility
    type: String,
    trim: true,
  },
  mediaUrls: [{  // NEW: Array for multiple images
    type: String,
    trim: true
  }],
  projectUrl: {
...
generationSource: { type: String, trim: true, default: 'upload' },
}, {
  _id: true,
  timestamps: true
});
...
// Add a pre-save hook to ensure mediaUrls is always set
portfolioItemSchema.pre('save', function (next) {
  // If mediaUrls is empty but mediaUrl exists, set mediaUrls from mediaUrl
  if ((!this.mediaUrls || this.mediaUrls.length === 0) && this.mediaUrl) {
    this.mediaUrls = [this.mediaUrl];
  }
  next();
});
...
// Add methods for referral handling
userSchema.methods.incrementReferralStats = async function (completed = false) {
  this.referralStats.totalReferrals += 1;
  if (completed) {
    this.referralStats.completedReferrals += 1;
  } else {
...
userSchema.methods.completeReferral = async function (referredUserId) {
  if (this.referralStats.pendingReferrals > 0) {
    this.referralStats.pendingReferrals -= 1;
    this.referralStats.completedReferrals += 1;
    await this.save();
  }
...
userSchema.methods.addReward = async function (points = 0, cashback = 0) {
  this.rewardPoints += points;
  this.rewardBalance += cashback;
  this.referralStats.referralRewards.points += points;
  this.referralStats.referralRewards.cashback += cashback;
  return this.save();
};
