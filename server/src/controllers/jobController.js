...
// @desc    Save job for candidate - FIXED: Removed application status check
...
exports.saveJob = async (req, res, next) => {
  try {
    ...
    // REMOVED: The check for job.isApplyEnabled
    // Saving jobs should be independent of application status
    // Users should be able to save jobs even if applications are closed
    ...
    // Increment job save count (safely)
...
// @desc    Unsave job for candidate - FIXED: Removed application status check
...
exports.unsaveJob = async (req, res, next) => {
  try {
    ...
    // Decrement job save count (safely, never below 0)
...
// @desc    Get saved jobs for candidate - UPDATED with better error handling
...
exports.getSavedJobs = async (req, res, next) => {
  try {
    ...
    console.log(`📋 Fetching ${user.savedJobs.length} saved jobs for user ${userId}`);

    // Get the actual jobs - don't filter by status or deadline for saved jobs
    // Users should see all their saved jobs regardless of status
    ...
    console.log(`✅ Found ${formattedJobs.length} saved jobs`);
