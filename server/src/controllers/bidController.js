...
//  FIX-B4  downloadUrl had the /api/v1 prefix which caused 404
//  FIX-B5  All queries that return bids to the client now populate bidder
//  🔔 NEW  Notification triggers for submitBid, updateBidStatus
// ══════════════════════════════════════════════════════════════════════
...
exports.submitBid = async (req, res) => {
    try {
        ...
        // 🔔 NOTIFICATION: Notify tender owner about new bid
        (async () => {
            try {
                await notificationService.create({
                    ...
                    body: `New bid submitted for "${tender.title}"`,
                    data: {
                        entityType: 'Tender',
                        entityId: tenderId,
                        screen: 'TenderDetail',
                        params: { tenderId, bidId: bid._id }
                    },
                    priority: 'high',
                    groupKey: `bid_received:${tenderId}`,
                    channels: { inApp: true, push: true, email: true }
                });
            } catch (notifErr) {
                console.warn('[Notification] submitBid:', notifErr.message);
            }
        })();
        // END NOTIFICATION
...
exports.updateBidStatus = async (req, res) => {
    try {
        ...
        // 🔔 NOTIFICATION: Notify bidder about status change
        (async () => {
            try {
                const statusNotifMap = {
                    'under_review':        { type: 'bid_status_changed', title: 'Bid under review', priority: 'normal' },
                    'shortlisted':         { type: 'bid_shortlisted', title: 'Bid shortlisted!', priority: 'high' },
                    'awarded':             { type: 'bid_awarded', title: 'Bid awarded! 🏆', priority: 'critical' },
                    'rejected':            { type: 'bid_rejected', title: 'Bid update', priority: 'normal' },
                    'interview_scheduled': { type: 'bid_status_changed', title: 'Interview scheduled', priority: 'high' }
                };
                const notifConfig = statusNotifMap[status];
                if (notifConfig && bid.bidder) {
                    await notificationService.create({
                        recipient: bid.bidder._id || bid.bidder,
                        actor: req.user._id,
                        type: notifConfig.type,
                        title: notifConfig.title,
                        body: `Your bid on "${tender.title}" status: ${status}`,
                        data: {
                            entityType: 'Tender',
                            entityId: tenderId,
                            screen: 'MyBids',
                            params: { tenderId, bidId }
                        },
                ...
                console.warn('[Notification] updateBidStatus:', notifErr.message);
            }
        })();
        // END NOTIFICATION
...
// ── FUNCTION 8 — submitEvaluationScore (first instance) ───────────────────────
...
// ── FUNCTION 9 — verifyCPOReturn (first instance) ─────────────────────────────
...
// ── FUNCTION 10 — updateComplianceChecklist (first instance) ───────────────────
...
// NOTE: The file has duplicate functions 8-10. These are the SECOND instances below.
// They are kept as-is from the original file. Only the first instances (above) are used.

// ── FUNCTION 8 (duplicate) — submitEvaluationScore ───────────────────────
exports.submitEvaluationScore = async (req, res) => {
    try {
        const { tenderId, bidId } = req.params;
...
// ── FUNCTION 9 (duplicate) — verifyCPOReturn ─────────────────────────────
exports.verifyCPOReturn = async (req, res) => {
    try {
        const { tenderId, bidId } = req.params;
        const { returnStatus, returnNotes } = req.body;
...
// ── FUNCTION 10 (duplicate) — updateComplianceChecklist ───────────────────
exports.updateComplianceChecklist = async (req, res) => {
    try {
        const { tenderId, bidId } = req.params;
        const { complianceChecklist } = req.body;
