...
describe('searchRoutes integration', () => {
  const app = getApp();

  describe('GET /api/v1/search/jobs', () => {
    it('returns 200 with matching active jobs', async () => {
      await createActiveJob({
        title: 'Route Searchable Developer Role',
        status: 'active',
      });

      const res = await request(app)
        .get('/api/v1/search/jobs')
        .query({ query: 'Route Searchable' });

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(Array.isArray(res.body.data)).toBe(true);
      expect(res.body.data.some((j) => j.title.includes('Route Searchable'))).toBe(true);
