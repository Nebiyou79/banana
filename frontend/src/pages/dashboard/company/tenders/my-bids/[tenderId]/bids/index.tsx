// pages/dashboard/company/tenders/my-bids/[tenderId]/bids/index.tsx
// Company owner — all bids received for a specific tender.
// FIX: BidCard now receives tenderId={tenderId} (was hardcoded tenderId={''})
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div className={`rounded-2xl ${colorClasses.bg.primary} border ${colorClasses.border.secondary} p-6 max-w-sm w-full shadow-2xl`}>
      ...
      <div className="flex gap-3">
        <button onClick={onCancel} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 ${colorClasses.border.secondary} ${colorClasses.text.primary} hover:opacity-80 transition-all`}>
          Cancel
        </button>
        <button onClick={onConfirm} disabled={loading} className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-[#0A2540] text-white hover:opacity-90 disabled:opacity-50 transition-all">
...
export default function CompanyTenderBidsListPage() {
  ...
  const handleReveal = async () => {
    ...
    try {
      ...
      toast({ title: '🔓 Bids revealed successfully!' });
      await refetch();
      setShowReveal(false);
    } catch (err: unknown) {
      ...
      <TenderDashboardLayout>
        <div className="p-8 flex items-center justify-center min-h-[400px]">
          <div className="flex flex-col items-center gap-3">
            <div className="w-10 h-10 rounded-full animate-spin" style={{ border: '2px solid #E5E5E5', borderTopColor: '#0A2540' }} />
  ...
  return (
    <TenderDashboardLayout>
      ...
      <div className="p-5 sm:p-8 space-y-8">
        {/* Header */}
        <div>
          <button onClick={() => router.push('/dashboard/company/tenders/my-bids')}
            className={`flex items-center gap-1 text-sm ${colorClasses.text.muted} hover:text-[#F1BB03] mb-3 transition-colors`}>
            ← My Bids
          </button>
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            {canReveal && (
              <button onClick={() => setShowReveal(true)}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold bg-[#0A2540] text-white hover:opacity-90 shadow-md flex-shrink-0 transition-all">
                🔓 Reveal Bids
              </button>
            )}
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
            ...
            {BID_STATUS_FILTERS.map(({ label, value }) => (
              <button key={value} onClick={() => setStatusFilter(value)}
                className={['px-3 py-1.5 rounded-full text-xs font-semibold transition-all whitespace-nowrap flex-shrink-0',
                  statusFilter === value ? 'bg-[#0A2540] text-white dark:bg-white dark:text-[#0A2540]'
          ...
          <div className="relative sm:ml-auto">
            ...
            <input type="text" placeholder="Search bidder or company…" value={search} onChange={(e) => setSearch(e.target.value)}
              className={`pl-9 pr-4 py-2 rounded-xl border ${colorClasses.border.secondary} ${colorClasses.bg.surface} ${colorClasses.text.primary} text-sm focus:outline-none focus:ring-2 focus:ring-[#F1BB03]/40 w-full sm:w-64`}
            />
          </div>
        </div>
        ...
        {bidsLoading ? (
          ...
          <div className={`rounded-2xl border ${colorClasses.border.secondary} ${colorClasses.bg.surface} flex flex-col items-center justify-center py-20 text-center px-6`}>
            ...
            <p className={`text-sm ${colorClasses.text.muted}`}>
              {allBids.length === 0 ? 'Share this tender to attract bidders.' : 'Try a different filter or search term.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredBids.map((bid) => (
              <BidCard
                key={bid._id}
                bid={bid}
                // FIX: was hardcoded tenderId={''} — now uses the actual tenderId from the route
                tenderId={tenderId}
                tenderWorkflowType={tender?.workflowType ?? 'open'}
                isBidsRevealed={isBidsRevealed}
                deadline={tender?.deadline}
                viewerRole="owner"
                onClick={() => router.push(`/dashboard/company/tenders/my-bids/${tenderId}/bids/${bid._id}`)}
              />
            ))}
          </div>
        )}
