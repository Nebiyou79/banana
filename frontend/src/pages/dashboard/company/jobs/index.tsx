...
const CompanyJobsPage: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['companyJobs', currentPage, retryCount],
  ...
  useEffect(() => {
    ...
    } else if (jobsResponse?.data?.length === 0) {
      // No jobs, total pages should be 1
      setTotalPages(1);
    }
  ...
  const stats = {
    ...
    activeJobs: jobs.filter((job: Job) => job.status === JobStatus.ACTIVE && !job.isExpired).length,
    ...
    closedJobs: jobs.filter((job: Job) => job.status === JobStatus.CLOSED || job.isExpired).length,
        ...
        <div className="min-h-screen flex items-center justify-center p-4">
          <div className="max-w-md w-full text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              {fetchError instanceof Error ? fetchError.message : 'There was an error loading your jobs. Please try again.'}
            </p>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                ...
                {pagination.totalResults > 0 && (
                  <span className="ml-2 text-sm font-medium text-blue-600 dark:text-blue-400">
                    ({pagination.totalResults} total job{pagination.totalResults !== 1 ? 's' : ''})
                  </span>
                )}
              </p>
            </div>
