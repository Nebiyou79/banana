// pages/dashboard/candidate/social/saved.tsx
...
function SavedPostsContent() {
    ...
    const [filters, setFilters] = useState<SavedPostsFilter>({
        sortBy: 'latest'
    });
    ...
    const loadInitialData = async () => {
        ...
        setPage(1);
        try {
            // Use getSavedPosts method from postService
            ...
            if (response.success && response.data) {
                // Load user interactions for saved posts
                const postsWithInteractions = user?._id
                    ? await postService.loadUserInteractionsForPosts(response.data, user._id)
                    : response.data;
                ...
                // Calculate stats
                const totalLikes = postsWithInteractions.reduce((sum, post) => sum + (post.stats?.likes || 0), 0);
                const totalComments = postsWithInteractions.reduce((sum, post) => sum + (post.stats?.comments || 0), 0);
                const uniqueAuthors = new Set(postsWithInteractions.map(post => post.author._id)).size;

                setStats({
                    totalPosts: postsWithInteractions.length,
                    totalLikes,
                    totalComments,
                    uniqueAuthors
                });
    ...
    const loadMorePosts = async () => {
        ...
        try {
            ...
            if (response.success && response.data && response.data.length > 0) {
                // Load user interactions for new posts
                const postsWithInteractions = user?._id
                    ? await postService.loadUserInteractionsForPosts(response.data, user._id)
                    : response.data;
    ...
    const handlePostUpdated = useCallback((updatedPost: Post) => {
        setPosts(prev => prev.map(post =>
            post._id === updatedPost._id ? { ...updatedPost, isSaved: true } : post
        ));
    }, []);
    ...
    const handlePostUnsaved = useCallback((postId: string) => {
        setPosts(prev => prev.filter(post => post._id !== postId));
        toast({
            variant: "info",
            title: "Unsaved",
    ...
    const handleViewProfile = (userId: string) => {
        window.open(`/social/profile/${userId}`, '_blank');
    };

    const handleFilterChange = (newFilters: Partial<SavedPostsFilter>) => {
    ...
    const handleRefresh = async () => {
        ...
        toast({
            variant: "default",
            title: "Saved posts refreshed",
            description: "Latest saved posts loaded"
        });
    };

    const handleExploreFeed = () => {
        router.push(`/dashboard/${role}/social`);
    };
    ...
    const renderEmptyState = () => (
        <div className="text-center py-12 px-4 md:py-16 animate-in fade-in-up duration-700">
            <div className={`w-16 h-16 md:w-20 md:h-20 mx-auto mb-6 rounded-full flex items-center justify-center animate-bounce animate-duration-[3s]`}
                ...
                <FolderHeart className="w-8 h-8 md:w-10 md:h-10" style={{ color: colors.primary }} />
            </div>
            <h3 className={`text-lg md:text-xl font-semibold mb-3 animate-in fade-in-up duration-500 ${getTextClasses('primary')}`}>
                {searchQuery ? "No saved posts found" : "Your saved collection is empty"}
            </h3>
            <p className={`text-sm md:text-base mb-6 md:mb-8 max-w-md mx-auto animate-in fade-in-up duration-500 animate-delay-200 ${getTextClasses('muted')}`}>
                {searchQuery
                    ? `No saved posts found for "${searchQuery}". Try a different search term.`
                    : "Save posts to access them later in one place. Your saved posts will appear here."}
            </p>
            {!searchQuery && (
                <div className="flex flex-col sm:flex-row gap-3 justify-center animate-in fade-in-up duration-500 animate-delay-400">
                    <Button
                        onClick={handleExploreFeed}
                        className={`px-5 py-2.5 md:px-6 md:py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 active:scale-95 ${getButtonClasses('primary')}`}
                        style={{
        ...
        <div className={`grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 ${statsLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-700`}>
            <div className={`rounded-xl p-4 border animate-in fade-in-up duration-500`} style={getCardStyle()}>
                <div className="flex items-center justify-between">
                    <div>
                        <p className={`text-xs md:text-sm mb-1 ${getTextClasses('muted')}`}>Saved Posts</p>
                        <p className={`text-xl md:text-2xl font-bold ${getTextClasses('primary')}`}>{stats.totalPosts}</p>
                    </div>
                    <BookmarkCheck className="w-6 h-6 md:w-8 md:h-8" style={{ color: colors.primary }} />
                </div>
            </div>
            ...
            <div className={`rounded-xl p-4 border animate-in fade-in-up duration-500 animate-delay-100`} style={getCardStyle()}>
                <div className="flex items-center justify-between">
                    <div>
                        <p className={`text-xs md:text-sm mb-1 ${getTextClasses('muted')}`}>Total Likes</p>
            ...
            <div className={`rounded-xl p-4 border animate-in fade-in-up duration-500 animate-delay-200`} style={getCardStyle()}>
                <div className="flex items-center justify-between">
                    <div>
                        <p className={`text-xs md:text-sm mb-1 ${getTextClasses('muted')}`}>Total Comments</p>
                        <p className={`text-xl md:text-2xl font-bold ${getTextClasses('primary')}`}>{stats.totalComments}</p>
                    </div>
                    <MessageSquare className="w-6 h-6 md:w-8 md:h-8" style={{ color: colors.secondary }} />
                </div>
            </div>
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {[
                                    { value: 'latest', label: 'Latest Saved', icon: Clock },
                                    { value: 'trending', label: 'Most Liked', icon: Flame },
                                    { value: 'popular', label: 'Most Comments', icon: TrendingUp }
                                ].map((option) => (
                                    <button
                                        key={option.value}
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {[
                                    { value: undefined, label: 'All Saved Posts' },
            ...
            <div className={isMobile ? '' : 'max-w-4xl mx-auto px-4 py-6 md:px-6 lg:px-8'}>
                ...
                {!isMobile && (
                    <div className="mb-6 md:mb-8">
                        <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-700`}>
                            <div>
                                <h1
                                    className={`text-2xl md:text-3xl font-bold mb-2 animate-text bg-linear-to-r from-primary to-secondary bg-clip-text`}
                                    style={{
                                        background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.secondary} 100%)`,
                        ...
                        <div className={`relative mb-6 ${searchLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} transition-all duration-700`}>
                            ...
                            <input
                                ...
                                placeholder="Search saved posts, authors, or hashtags..."
                ...
                {isMobile && (
                    <div className="sticky top-0 z-30 border-b px-4 py-3 backdrop-blur-sm" style={getCardStyle()}>
                        <div className="flex items-center justify-between mb-3">
                            <h1
                                className={`text-xl font-bold bg-linear-to-r from-primary to-secondary bg-clip-text`}
                                style={{
                                    background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.secondary} 100%)`,
                                    WebkitBackgroundClip: 'text',
                                    WebkitTextFillColor: 'transparent'
                    ...
                    {loading && posts.length === 0 ? (
                        ...
                        <div className={isMobile ? 'space-y-4' : 'space-y-4 md:space-y-6'}>
                            {filteredPosts.map((post, index) => (
                                ...
                                >
                                    <PostCard
                                        post={{ ...post, isSaved: true }} // Ensure saved flag is set
                    ...
                    {filteredPosts.length > 0 && hasMore && (
                        <div ref={loadMoreRef} className="pt-6 md:pt-8 text-center">
                            <div className={`inline-flex items-center gap-2 ${getTextClasses('muted')}`}>
                                ...
                                <span className="text-sm">Loading more saved posts...</span>
                            </div>
                        </div>
                    )}
                    ...
                    {filteredPosts.length > 0 && !hasMore && !loading && (
                        ...
                        >
                            <div className={`w-10 h-10 md:w-12 md:h-12 mx-auto mb-3 md:mb-4 rounded-full flex items-center justify-center animate-bounce animate-duration-[3s]`}
                                ...
                                <BookmarkCheck className="w-5 h-5 md:w-6 md:h-6" style={{ color: colors.primary }} />
                            </div>
                            ...
                            <p className={`text-xs mt-1 ${getTextClasses('muted')}`}>
                                Explore the feed to save more interesting content
                            </p>
                        </div>
                    )}
                </div>
                        ...
                        <div className={`flex flex-wrap items-center justify-between text-sm ${getTextClasses('muted')}`}>
                            <div className="flex items-center gap-4">
                                <span className="flex items-center gap-1">
                                    <Bookmark className="w-3 h-3" style={{ color: colors.primary }} />
...
export default function SavedPostsPage() {
    return (
        <>
            <Head>
                <title>Saved Posts | Banana Social</title>
                <meta name="description" content="Access your saved posts in one place" />
            </Head>
