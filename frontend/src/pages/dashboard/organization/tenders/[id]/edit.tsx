...
'use client';
// pages/dashboard/organization/tenders/[id]/edit.tsx
// ─── Page 4.4 — Organization: Tenders Edit ───────────────────────────────────
// Edit an organization-posted tender. Identical to Company My Tenders Edit (Page 1.4)
// with route and navigation differences.
//
// KEY DIFFERENCES from Company Edit (Page 1.4):
//   • Route: /dashboard/organization/tenders/[id]/edit
//   • On success navigate: /dashboard/organization/tenders/[id]
//   • Addendum CTA navigate: /dashboard/organization/tenders/[id]?openAddendum=true
//   • Breadcrumbs: Organization Dashboard > Tenders > [Tender Title] > Edit
//
// EVERYTHING ELSE identical to Page 1.4:
//   • Same freelance edit flow (always editable, pre-fill from useFreelanceTenderEditData)
//   • Same professional draft guard (non-draft = read-only banner with addendum CTA)
//   • Same hooks: useFreelanceTenderEditData, useProfessionalTenderEditData,
//                 useUpdateFreelanceTender, useUpdateProfessionalTender
//   • Same FormData serialization rules (JSON.stringify nested objects, join arrays)

import React, { useCallback } from 'react';
import { useParams, useSearchParams, useRouter } from 'next/navigation';
import { colorClasses } from '@/utils/color';

import { useFreelanceTenderEditData } from '@/hooks/useFreelanceTender';
import { useProfessionalTenderEditData } from '@/hooks/useProfessionalTender';
...
// Shown when professional tender status !== 'draft'. Organizations use Addenda.
function NonEditableBanner({ tenderId, status, title }: { tenderId: string; status: string; title: string }) {
  const router = useRouter();

  // ── DIFFERENCE: routes use ROUTE_BASE = /dashboard/organization/tenders
  const goToDetail = useCallback(
    () => router.push(`${ROUTE_BASE}/${tenderId}?type=professional`),
    [router, tenderId]
  );
  const goToAddendum = useCallback(
    () => router.push(`${ROUTE_BASE}/${tenderId}?type=professional&openAddendum=true`),
    [router, tenderId]
  );
    ...
    <div className={`min-h-screen ${colorClasses.bg.surface}`}>
      <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          <h1 className={`text-2xl font-bold truncate ${colorClasses.text.primary}`}>{title}</h1>
          <TenderStatusBadge status={status as any} size="md" showDot />
        </div>

        <div className={`rounded-2xl border-2 p-8 sm:p-10 ${colorClasses.bg.primary} border-[#F59E0B] dark:border-[#D97706]`}>
          <div className={`mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl ${colorClasses.bg.amberLight}`}>
            <svg className={`w-8 h-8 ${colorClasses.text.amber}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 rounded-xl border p-5 mb-8 ${colorClasses.bg.surface} ${colorClasses.border.secondary}`}>
            {[
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              ...
              className={`inline-flex items-center justify-center gap-2.5 rounded-xl px-6 py-3 text-sm font-bold hover:opacity-90 hover:-translate-y-0.5 shadow-md ${colorClasses.bg.darkNavy} ${colorClasses.text.inverse}`}
            >
            ...
            <button
              onClick={goToDetail}
              className={`inline-flex items-center justify-center gap-2 rounded-xl border px-6 py-3 text-sm font-semibold hover:opacity-80 ${colorClasses.border.primary} ${colorClasses.text.primary}`}
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
              </svg>
...
function FreelanceEditPage({ id }: { id: string }) {
  ...
  // ── DIFFERENCE: navigate to org route
  const goToDetail = useCallback(
    () => router.push(`${ROUTE_BASE}/${id}?type=freelance`),
    [router, id]
  );
  ...
  return (
    <div className={`min-h-screen ${colorClasses.bg.surface}`}>
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Breadcrumb */}
        <nav className={`flex items-center gap-1.5 text-xs mb-6 ${colorClasses.text.muted}`}>
          <button onClick={() => router.push('/dashboard/organization')} className="hover:opacity-70">Organization Dashboard</button>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          <button onClick={() => router.push(ROUTE_BASE)} className="hover:opacity-70">Tenders</button>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          <button onClick={goToDetail} className="hover:opacity-70 truncate max-w-[160px]">{(editData as any).title ?? 'Tender'}</button>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          <span className={`font-medium ${colorClasses.text.primary}`}>Edit</span>
        </nav>

        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${colorClasses.bg.indigoLight} ${colorClasses.text.indigo}`}>
              🧑‍💻 Freelance Tender
            </span>
            <TenderStatusBadge status={(editData as any).status ?? 'draft'} size="sm" showDot />
          </div>
          <h1 className={`text-2xl sm:text-3xl font-bold tracking-tight mt-2 ${colorClasses.text.primary}`}>Edit Tender</h1>
          <p className={`text-sm mt-1 ${colorClasses.text.muted}`}>Freelance tenders can be edited at any time regardless of status.</p>
        </div>

        <div className={`flex items-start gap-3 rounded-xl border p-4 mb-6 ${colorClasses.bg.blueLight} border-[#4DA6FF]/30`}>
          ...
          <p className={`text-sm ${colorClasses.text.blue600}`}>
...
function ProfessionalEditPage({ id }: { id: string }) {
  ...
  // ── DIFFERENCE: navigate to org route
  const goToDetail = useCallback(
    () => router.push(`${ROUTE_BASE}/${id}?type=professional`),
    [router, id]
  );
  ...
  // ── Draft guard: non-draft professional → read-only banner + addendum CTA
  if (!isDraft) {
    return (
      <NonEditableBanner
        tenderId={id}
    ...
    <div className={`min-h-screen ${colorClasses.bg.surface}`}>
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className={`flex items-center gap-1.5 text-xs mb-6 ${colorClasses.text.muted}`}>
          ...
          <button onClick={() => router.push(ROUTE_BASE)} className="hover:opacity-70">Tenders</button>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          <button onClick={goToDetail} className="hover:opacity-70 truncate max-w-[160px]">{editData.title ?? 'Tender'}</button>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          <span className={`font-medium ${colorClasses.text.primary}`}>Edit</span>
        </nav>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${colorClasses.bg.blueLight} ${colorClasses.text.blue600}`}>
              🏢 Professional Tender
            </span>
            <TenderStatusBadge status={status as any} size="sm" showDot />
          </div>
          <h1 className={`text-2xl sm:text-3xl font-bold tracking-tight mt-2 ${colorClasses.text.primary}`}>Edit Tender</h1>
          <p className={`text-sm mt-1 ${colorClasses.text.muted}`}>Draft tenders can be fully edited. Once published, use Addenda for changes.</p>
        </div>

        <div className={`flex items-start gap-3 rounded-xl border p-4 mb-6 ${colorClasses.bg.amberLight} border-[#F59E0B]/40`}>
...
export default function OrganizationEditTenderPage() {
  ...
  const isDetecting = (!typeParam && isFreelanceLoading) || (!typeParam && isProfessionalLoading);

  if (isDetecting && !resolvedType) return (
    <DashboardLayout requiredRole="organization">
      <PageSkeleton />
    </DashboardLayout>
  );
  if (resolvedType === 'freelance') return (
    <DashboardLayout requiredRole="organization">
      <FreelanceEditPage id={id} />
    </DashboardLayout>
  );
  if (resolvedType === 'professional') return (
    <DashboardLayout requiredRole="organization">
    ...
    <DashboardLayout requiredRole="organization">
      <div className={`min-h-screen flex flex-col items-center justify-center gap-4 ${colorClasses.bg.surface}`}>
        ...
        <button
          ...
          className={`mt-2 inline-flex items-center gap-2 rounded-xl border px-5 py-2.5 text-sm font-semibold hover:opacity-80 ${colorClasses.border.primary} ${colorClasses.text.primary}`}
        >
          Back to Tenders
        </button>
      </div>
