...
// pages/dashboard/organization/applications/[id].tsx - PREMIUM TEAL VERSION
...
const OrganizationApplicationDetailPage: React.FC = () => {
  ...
  const getOpportunityTypeIcon = (job: any) => {
    const opportunityType = job?.opportunityType || 'volunteer';
    switch (opportunityType) {
      case 'volunteer':
        return <Heart className="h-6 w-6 text-teal-400" />;
      case 'internship':
        return <Target className="h-6 w-6 text-cyan-400" />;
      case 'fellowship':
  ...
  const getOpportunityTypeLabel = (job: any) => {
    const opportunityType = job?.opportunityType || 'volunteer';
    switch (opportunityType) {
      case 'volunteer':
      ...
      case 'fellowship':
        return 'Fellowship Program';
      default:
        return 'Community Opportunity';
    }
  };
  ...
  const formattedApplication = applicationService.formatApplication(applicationData);
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h1 className="text-3xl font-bold text-gray-900">
                          {candidate?.name || applicationData.userInfo?.name || 'Community Applicant'}
                        </h1>
                        <p className="text-gray-600 text-lg mt-1">
                          Applied for {job?.title || 'Opportunity'}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mt-4">
                    ...
                    <div className="flex items-center gap-2 bg-white/30 px-3 py-1.5 rounded-full border border-white/40">
                      ...
                      <span>{getOpportunityTypeLabel(job)}</span>
                    </div>
                    <div className="flex items-center gap-2 bg-white/30 px-3 py-1.5 rounded-full border border-white/40">
                      <User className="h-4 w-4" />
                      <span>ID: {applicationData._id.slice(-8)}</span>
                    </div>
                    <div className="flex items-center gap-2 bg-gradient-to-r from-teal-400 to-cyan-400 px-3 py-1.5 rounded-full text-white">
          ...
          <GlassCard className="p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="text-sm text-gray-600">
                <p>Community opportunity management • Premium Glass UI • Teal Theme</p>
              </div>
