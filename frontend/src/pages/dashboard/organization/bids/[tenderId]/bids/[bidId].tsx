// pages/dashboard/organization/bids/[tenderId]/bids/[bidId].tsx
// Organization owner — single bid deep-evaluation page.
// Layout: BidHeader → SealedBidBanner (if sealed) → OwnerBidDetails (4-tab) + ActionSidebar.
// DashboardLayout. Teal accent #0D9488 throughout.
// ActionSidebar labels: "Committee Notes" instead of "Owner Notes".
// OwnerBidDetails variant="organization" → "Committee Evaluation" label.
...
// ─────────────────────────────────────────────────────────────────────────────
// Action Sidebar — teal accent, committee-specific labels
// ─────────────────────────────────────────────────────────────────────────────
...
const ActionSidebar = ({ bid, tenderId }: { bid: Bid; tenderId: string }) => {
  ...
  return (
    <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
      ...
      {/* Committee notes — auto-saves on blur */}
      <div className={`rounded-2xl border ${colorClasses.border.secondary} ${colorClasses.bg.primary} p-4`}>
        ...
        <textarea
          ...
          placeholder="Add private committee notes about this bid…"
          className={`w-full rounded-xl border ${colorClasses.border.secondary} ${colorClasses.bg.surface} ${colorClasses.text.primary} text-xs px-3 py-2.5 resize-none focus:outline-none focus:ring-2 focus:ring-[#0D9488]/40`}
        />
      </div>
...
export default function OrgOwnerBidDetailPage() {
  ...
  const backPath = `/dashboard/organization/bids/${tenderId}/bids`;
    ...
    <DashboardLayout requiredRole="organization">
      ...
      <div className="p-5 sm:p-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          {/* Main — OwnerBidDetails with org variant = committee eval label */}
