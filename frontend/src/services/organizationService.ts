...
// Get public organization profile - WITH FALLBACK
getPublicOrganization: async (id: string): Promise<OrganizationProfile | null> => {
  try {
    if (!id) {
      console.warn('Organization ID is required for public profile');
      return null;
    }

    const response = await api.get<ApiResponse<OrganizationProfile>>(`/organization/public/${id}`);

    if (!response.data.success || !response.data.data) {
      console.warn('Public organization endpoint returned error, returning null');
      return null;
    }
    ...
    if (error.response?.status === 404) {
      console.warn('Public organization endpoint not available (404) - this is expected if backend routes not updated');
      return null;
    }
    
    // For other errors, log but don't throw
    console.warn('Failed to fetch public organization profile:', error.message);
    return null;
  }
},
  // Utility functions
