...
// Main Job Interface - UPDATED WITH AVATAR FIELDS
export interface Job {
  ...
  // Company fields (for company jobs) - UPDATED WITH AVATAR
  company?: {
    _id: string;
    name: string;
    logoUrl?: string;
  ...
  // Organization fields (for organization opportunities) - UPDATED WITH AVATAR
  organization?: {
    _id: string;
    name: string;
    organizationType?: string;
  ...
  // NEW AVATAR FIELDS FOR QUICK ACCESS
...
// Job Form Data Interface with Avatar Fields
export interface JobFormData {
  ...
  // Optional owner info (will be populated automatically during creation)
  ownerAvatarUrl?: string;
  ownerAvatarPublicId?: string;
  ownerName?: string;
  ownerVerified?: boolean;
...
// In jobService.ts, update the JobFilters interface:
// Update JobFilters interface to include company filtering
export interface JobFilters {
  demographicSex?: 'male' | 'female' | 'any';
  ...
  currency?: 'ETB' | 'USD' | 'EUR' | 'GBP';
  salaryMode?: SalaryMode;
  company?: string; // Can be single ID or comma-separated IDs
  companyName?: string; // For searching by company name
  featured?: boolean;
  urgent?: boolean;
  jobType?: 'company' | 'organization';
  status?: JobStatus;
...
export const jobService = {
   ...
   * @param params - Filter parameters including company filtering
  ...
  getJobs: async (params?: JobFilters): Promise<JobsResponse> => {
    try {
      ...
      // Process jobs to ensure all fields are properly formatted
      const processedData = response.data.data.map(job =>
        jobService.processJobResponse(job)
      );
      ...
      if (error.response?.status === 404) {
        ...
        return {
          ...
          pagination: {
            ...
            totalPages: 1,
            totalResults: 0,
            resultsPerPage: params?.limit || 12
          }
        };
      }

      // Use the existing handleApiError for other errors
...
getJob: async (id: string): Promise<Job> => {
  try {
    ...
    if (!objectIdRegex.test(id)) {
      ...
      throw new Error(`Invalid job ID format. Job not found.`);
    }
    ...
    // Process the job to add computed fields and check expiration
    const processedJob = jobService.processJobResponse(response.data.data);
    
    console.log('✅ Job fetched successfully:', processedJob._id);
    return processedJob;
    ...
    if (error.response?.status === 404) {
      ...
      throw new Error('Invalid job ID format. The job may not exist.');
    }
...
getCompanyJobs: async (params?: { page?: number; limit?: number; status?: string }): Promise<CompanyJobsResponse> => {
  try {
    console.log('🔍 Fetching company jobs with params:', params);
    ...
    // Process each job to ensure computed fields are set
    if (response.data.data) {
  ...
  /**
   * Create a new job with owner avatar
  ...
  createJob: async (data: JobFormData): Promise<Job> => {
    try {
      ...
      console.log('📤 Preparing job data with owner information...');

      try {
        // Fetch current user's profile to include avatar
        const profile = await profileService.getProfile();
        console.log('👤 User profile fetched for job creation:', {
          userId: profile.user._id,
          name: profile.user.name,
          role: profile.user.role,
          hasAvatar: !!profile.avatar?.secure_url
        });

        // Get avatar URL from profile
        const avatarUrl = profileService.getAvatarUrl(profile);
        const avatarPublicId = profile.avatar?.public_id;

        // Prepare enhanced job data with owner info
        const jobDataWithOwner = {
          ...data,
          // Include owner information
          createdBy: profile.user._id,
          ownerType: profile.user.role === 'organization' ? 'organization' : 'company',
          // Add avatar data
          ownerAvatarUrl: avatarUrl,
          ownerAvatarPublicId: avatarPublicId,
          ownerName: profile.user.name,
          ownerVerified: profile.isVerified || profile.user.verificationStatus === 'verified'
        };

        console.log('📤 Sending job data to backend with owner info:', {
          title: jobDataWithOwner.title,
          ownerName: jobDataWithOwner.ownerName,
          ownerType: jobDataWithOwner.ownerType,
          hasAvatar: !!jobDataWithOwner.ownerAvatarUrl
        });

        const response = await api.post<JobResponse>('/job', jobDataWithOwner);

        if (!response.data.success || !response.data.data) {
          throw new Error(response.data.message || 'Failed to create job');
        }

        // Process the response to ensure avatar fields are set
        const processedJob = jobService.processJobResponse(response.data.data);

        handleSuccess('Job created successfully');

        console.log('✅ Job created with avatar data:', {
          jobId: processedJob._id,
        ...
        console.warn('⚠️ Could not fetch profile, creating job without owner info:', profileError.message);

        // Fallback: create job without owner avatar
  ...
  /**
   * Update an existing job with owner avatar
   * @param id - Job ID
  ...
  updateJob: async (id: string, data: Partial<JobFormData>): Promise<Job> => {
    try {
      ...
      // Try to get current profile for avatar updates
      try {
        const profile = await profileService.getProfile();
        const avatarUrl = profileService.getAvatarUrl(profile);
        const avatarPublicId = profile.avatar?.public_id;

        // Include updated owner info if needed
        const updateData = {
          ...data,
          // Only update owner info if not already set or if avatar changed
          ...(avatarUrl && {
            ownerAvatarUrl: avatarUrl,
            ownerAvatarPublicId: avatarPublicId,
            ownerName: profile.user.name
        ...
        const response = await api.put<JobResponse>(`/job/${id}`, updateData);
        ...
        console.log('✅ Job updated with avatar:', {
          jobId: processedJob._id,
          ownerAvatarUpdated: !!avatarUrl
        });

        return processedJob;
      } catch (profileError) {
        console.warn('⚠️ Could not fetch profile, updating job without avatar refresh');

        // Fallback: update without avatar refresh
  ...
  /**
   * Fetch jobs for candidates with advanced filtering
  ...
  getJobsForCandidate: async (params?: JobFilters): Promise<JobsResponse> => {
    try {
      ...
      // Transform the response to match JobsResponse interface
      return {
        success: true,
        ...
        pagination: {
          ...
          resultsPerPage: response.data.pagination.limit
        }
      };
    } catch (error: any) {
      console.error('Failed to fetch jobs for candidate:', error);
  ...
  /**
   * Save a job for candidate - FIXED: Better error handling
  ...
  saveJob: async (jobId: string): Promise<{ saved: boolean }> => {
    try {
      ...
      if (!response.data.success) {
        // If job is already saved, treat as success
        if (response.data.message?.toLowerCase().includes('already saved')) {
          return { saved: true };
        }
      ...
      if (error.response?.status === 400) {
        ...
        // If job is already saved, return success
        if (message.toLowerCase().includes('already saved')) {
          return { saved: true };
        }

        // If applications are closed, still allow saving (this is a backend issue)
        if (message.toLowerCase().includes('applications are closed')) {
          console.warn('Backend incorrectly blocking save due to application status');
          // Return success anyway since the UI already updated optimistically
          return { saved: true };
        }
      }
  ...
  /**
   * Unsave a job for candidate - FIXED: Better error handling
  ...
  unsaveJob: async (jobId: string): Promise<{ saved: boolean }> => {
    try {
      ...
      if (!response.data.success) {
        // If job is not saved, treat as success
        if (response.data.message?.toLowerCase().includes('not saved')) {
          return { saved: false };
        }
      ...
      if (error.response?.status === 400) {
        ...
        // If job is not saved, return success
        if (message.toLowerCase().includes('not saved') ||
  ...
  /**
   * Fetch saved jobs for candidate - FIXED: Better error handling
...
canJobAcceptApplications: (job: Job): boolean => {
  // If apply is explicitly disabled
  if (job.isApplyEnabled === false) return false;
  
  // If job status is not active
  ...
  if (job.applicationDeadline) {
    ...
    // Check if deadline is today (still valid)
    // You might want to allow applications until end of day
    const isSameDay = deadlineDate.toDateString() === now.toDateString();
    if (isSameDay && deadlineDate.getTime() < now.getTime()) {
      return false;
    }
  }
  
  // Check if enough candidates are needed
  const candidatesRemaining = jobService.calculateCandidatesRemaining(job);
  if (candidatesRemaining <= 0) return false;
...
getApplicationStatusInfo: (job: Job): ApplicationInfo => {
  ...
  if (!job.isApplyEnabled) {
    ...
    reason = ApplicationStatusReason.CLOSED;
  ...
  /**
  * Process API job response to ensure all avatar fields are properly set
...
processJobResponse: (job: any): Job => {
  ...
  // ========== JOB STATUS & EXPIRATION CHECK ==========
  // Check if job is expired based on deadline
  const now = new Date();
  const hasDeadline = job.applicationDeadline && job.applicationDeadline !== null;
  const isDeadlinePassed = hasDeadline && new Date(job.applicationDeadline) < now;
  
  // Auto-update status to CLOSED if deadline passed and job is ACTIVE
  if (isDeadlinePassed && job.status === JobStatus.ACTIVE) {
    console.log(`⚠️ Job ${job._id} has passed its deadline, marking as CLOSED`);
    job.status = JobStatus.CLOSED;
  }

  // ========== AVATAR PROCESSING ==========
  // Determine owner info based on jobType
  ...
  // Set ownerType
  job.ownerType = job.jobType === 'organization' ? 'Organization' : 'Company';

  // Set ownerVerified status
  job.ownerVerified = ownerInfo?.verified || false;

  // Avatar URL processing (simplified)
  if (ownerInfo?.avatar) {
    job.ownerAvatarUrl = ownerInfo.avatar;
  } else if (ownerInfo?.logoUrl) {
    job.ownerAvatarUrl = ownerInfo.logoUrl;
  } else if (job.jobType === 'organization' && job.organization?.logoFullUrl) {
    job.ownerAvatarUrl = job.organization.logoFullUrl;
  } else {
    ...
    const backgroundColor = job.jobType === 'organization' ? '4F46E5' : '10B981';
    job.ownerAvatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&background=${backgroundColor}&color=fff&size=150`;
  }
  ...
  // Add application info with expiration awareness
  ...
  // Set isActive based on status and deadline
  job.isActive = job.status === JobStatus.ACTIVE && 
    (!hasDeadline || !isDeadlinePassed);
  
  // Set isExpired flag
  job.isExpired = isDeadlinePassed;
  
  // Set canAcceptApplications
  job.canAcceptApplications = jobService.canJobAcceptApplications(job);
  ...
  if (job.jobType === 'organization' && job.opportunityType) {
    ...
    job.displayType = displayTypes[job.opportunityType] || 'Opportunity';
  } else {
    job.displayType = 'Job';
  }
  ...
  /**
   * Get avatar URL for job owner with fallback options
   * @param job - Job object
   * @param options - Options for avatar generation
   * @returns Avatar URL string
   */
  getOwnerAvatarUrl: (job: Job, options?: {
    size?: number;
    ...
    // CRITICAL FIX: Check for REAL avatar first (not placeholder)

    // Priority 1: Direct ownerAvatarUrl from job (if it's a real URL)
    if (job.ownerAvatarUrl && !job.ownerAvatarUrl.includes('ui-avatars.com')) {
      return job.ownerAvatarUrl;
    }

    // Priority 2: Check company/organization for avatar
    const ownerInfo = job.jobType === 'organization' ? job.organization : job.company;
    const ownerName = ownerInfo?.name || job.ownerName || 'Unknown';

    // Priority 3: Check ownerInfo.avatar (Cloudinary)
    if (ownerInfo?.avatar) {
      return ownerInfo.avatar;
    }

    // Priority 4: Check ownerInfo.logoUrl
    if (ownerInfo?.logoUrl) {
      return ownerInfo.logoUrl;
    }

    // Priority 5: Check organization.logoFullUrl
    if (job.jobType === 'organization' && job.organization?.logoFullUrl) {
      return job.organization.logoFullUrl;
    }

    // Priority 6: Check createdBy for avatar
    if (job.createdBy && typeof job.createdBy === 'object') {
      const createdBy = job.createdBy as any;
      if (createdBy.avatar) {
        return createdBy.avatar;
      }
    }

    // Priority 7: Use existing ownerAvatarUrl even if it's a placeholder
    if (job.ownerAvatarUrl) {
      // Resize placeholder if needed
      if (job.ownerAvatarUrl.includes('ui-avatars.com') && defaultSize !== 150) {
        return job.ownerAvatarUrl.replace('size=150', `size=${defaultSize}`);
      }
      return job.ownerAvatarUrl;
    }

    // Priority 8: Generate new placeholder if allowed
    if (usePlaceholder) {
      const initials = ownerName
        .split(' ')
      ...
      const backgroundColor = job.jobType === 'organization' ? '4F46E5' : '10B981';
      return `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&background=${backgroundColor}&color=fff&size=${defaultSize}`;
    }

    // Return empty string if no avatar available and placeholder not allowed
    return '';
  },
