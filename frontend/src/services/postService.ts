...
export const postService = {
  ...
  getFeedPosts: async (params?: FeedParams, currentUserId?: string): Promise<PostsResponse> => {
    try {
      ...
      // Ensure data is an array, filter out posts with null/missing authors, and fix media URLs
      const postsData = Array.isArray(response.data.data) ? response.data.data : [];
      const validPosts = postsData.filter((post: any) => post?.author?._id != null);
      if (validPosts.length !== postsData.length) {
        console.warn(`⚠️ Filtered out ${postsData.length - validPosts.length} post(s) with null/missing author`);
      }
      const fixedPosts = validPosts.map((post: any) => postService.fixPostMediaUrls(post));
