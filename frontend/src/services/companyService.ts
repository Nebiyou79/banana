...
// Get public company profile - WITH FALLBACK
getPublicCompany: async (id: string): Promise<CompanyProfile | null> => {
  try {
    if (!id) {
      console.warn('Company ID is required for public profile');
      return null;
    }

    const response = await api.get<ApiResponse<CompanyProfile>>(`/company/public/${id}`);

    if (!response.data.success || !response.data.data) {
      console.warn('Public company endpoint returned error, returning null');
      return null;
    }
    ...
    if (error.response?.status === 404) {
      console.warn('Public company endpoint not available (404) - this is expected if backend routes not updated');
      return null;
    }
    
    // For other errors, log but don't throw
    console.warn('Failed to fetch public company profile:', error.message);
    return null;
  }
},
