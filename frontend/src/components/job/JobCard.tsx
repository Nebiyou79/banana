...
const Avatar: React.FC<AvatarProps> = ({
  ...
  return (
    <div className={`${sizeClasses[size]} rounded-xl overflow-hidden ${bordered ? 'border-2' : ''}`}
      ...
      <img
        ...
        onError={(e) => {
          ...
          target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&background=${bgColor}&color=fff&size=${size === 'sm' ? 32 : size === 'md' ? 48 : 64}`;
        }}
      />
    </div>
  );
...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const isExpired = job.isExpired || (job.applicationDeadline && new Date(job.applicationDeadline) < new Date());

  const getStatusConfig = (status: JobStatus) => {
    // If job is expired, show as CLOSED
    if (isExpired && status === JobStatus.ACTIVE) {
      return {
            ...
            <div className="flex items-start justify-between mb-1.5 sm:mb-2">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center space-x-1.5 sm:space-x-2 mt-1">
                  ...
                  {/* Deadline warning */}
                  {daysUntilDeadline && daysUntilDeadline <= 3 && daysUntilDeadline > 0 && !isExpired && (
                    <span className="inline-flex items-center px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200">
                      <Clock className="w-3 h-3 mr-1" />
