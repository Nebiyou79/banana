...
import { Sparkles, Zap, TrendingUp, Bell, ChevronRight, ChevronUp, ChevronLeft, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
function SocialDashboardContent({
  ...
  const [adSidebarOpen, setAdSidebarOpen] = useState(false); // NEW: Control AdSidebar visibility
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      ...
      // NEW: Auto-hide AdSidebar below 1320px, show above
      if (width < 1320) {
        setAdSidebarOpen(false);
      } else {
        setAdSidebarOpen(true);
      }
    };
  ...
  if (requiredRole && user?.role !== requiredRole) {
    return (
      <div className="min-h-screen flex items-center justify-center relative animate-in fade-in duration-500" style={getPageBgStyle()}>
        {/* Animated background */}
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-linear-to-br from-red-500/10 via-orange-500/10 to-red-500/10 animate-pulse" />
          <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-transparent via-red-500 to-transparent animate-shimmer animate-duration-[2000ms] animate-infinite" />
        </div>
  ...
  const getSidebarWidth = () => {
    if (windowWidth < 1320) return 'w-[320px]'; // NEW: Reduced width for 1320-1500 range
    if (windowWidth < 1500) return 'w-[340px]'; // NEW: Slightly reduced width
    if (windowWidth < 1536) return 'w-[380px]';
    return 'w-[420px]';
  };

  const getMainMargin = () => {
    if (!adSidebarOpen || windowWidth < 1320) return ''; // NEW: No margin when AdSidebar hidden
    if (windowWidth < 1500) return 'lg:mr-[340px]'; // NEW: Adjusted margin
  ...
  // NEW: Determine if AdSidebar should be visible as drawer
  const showAdSidebarAsDrawer = adSidebarOpen && windowWidth < 1320;
  const showAdSidebarFixed = adSidebarOpen && windowWidth >= 1320;
  ...
  return (
    <div className={`flex min-h-screen relative overflow-hidden animate-in fade-in duration-500 transition-all duration-300 ${pageLoaded ? 'opacity-100' : 'opacity-0'}`} style={getPageBgStyle()}>
      ...
      {/* AD SIDEBAR DRAWER OVERLAY (for widths < 1320px) */}
      {showAdSidebarAsDrawer && (
        <>
          <div
            className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300"
            onClick={() => setAdSidebarOpen(false)}
          />
          <div className="fixed inset-y-0 right-0 z-50 animate-in slide-in-from-right-80 duration-300 w-[320px]">
            <div className="h-full overflow-y-auto pt-24 pb-8 px-3 bg-white dark:bg-gray-900"
              style={{
                scrollbarWidth: 'thin',
                scrollbarColor: `${colors.primary + '30'} transparent`,
              }}
            >
              {/* Drawer content - same as AdSidebar */}
              <div className="space-y-5">
                {renderAdSidebarContent()}
              </div>
            </div>
          </div>
        </>
      ...
      {/* DESKTOP FIXED AD SIDEBAR (for widths >= 1320px) */}
      {showAdSidebarFixed && (
        <div className={`hidden xl:block fixed right-0 top-0 bottom-0 z-20 animate-in slide-in-from-right-0 duration-500 ${getSidebarWidth()}`}>
          <div className={`h-full overflow-y-auto pt-24 pb-8 ${windowWidth < 1500 ? 'px-3' : 'px-4'} ${windowWidth >= 1536 ? 'px-5' : ''}`}
            style={{
              scrollbarWidth: 'thin',
              scrollbarColor: `${colors.primary + '30'} transparent`,
            }}
      ...
      {/* FLOATING AD SIDEBAR TOGGLE BUTTON (shown when AdSidebar is hidden on desktop) */}
      {!isMobile && !adSidebarOpen && windowWidth >= 1024 && ads.length > 0 && (
        <button
          onClick={() => setAdSidebarOpen(true)}
          className="fixed right-4 bottom-4 z-40 p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 active:scale-95 animate-in fade-in-up duration-500"
          style={{
            background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.secondary} 100%)`,
            color: 'white'
          }}
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${!isMobile ? `lg:ml-80 ${getMainMargin()}` : ''}`}>
        ...
        {/* AD SIDEBAR TOGGLE BUTTON IN NAVBAR AREA (for mobile/desktop when hidden) */}
        {ads.length > 0 && !adSidebarOpen && (
          <div className="absolute top-24 right-4 z-30 lg:hidden">
            <button
              onClick={() => setAdSidebarOpen(true)}
              className="p-2.5 rounded-lg shadow-md hover:shadow-lg transition-all duration-300"
              style={getCardStyle()}
            >
              <PanelLeftOpen className="w-4 h-4" style={{ color: colors.primary }} />
            </button>
          </div>
        )}
  ...
  /* ------------------- NEW: Helper function for AdSidebar content ------------------- */
  function renderAdSidebarContent() {
    return (
      <>
        {/* Profile Card - RESPONSIVE */}
        {userProfile && (
          <div
            className={`rounded-xl shadow-sm ${windowWidth < 1500 ? 'p-3' : 'p-4'} animate-in slide-in-from-right-0 duration-500`}
            style={{
              ...getCardStyle(),
              border: `1px solid ${colors.cardBorderLight || '#e5e7eb'}`
            }}
          >
            <div className={`flex items-center ${windowWidth < 1500 ? 'gap-3' : 'gap-4'}`}>
              {/* Avatar */}
              <div className="relative">
                <div
                  className={`${windowWidth < 1500 ? 'w-14 h-14' : 'w-16 h-16'} rounded-lg flex items-center justify-center overflow-hidden shadow-lg border-2`}
                  style={{
                ...
                >
                  {getAvatarUrl() ? (
                    ...
                    <span className={`${windowWidth < 1500 ? 'text-base' : 'text-lg'} text-white font-bold`}>
              ...
              <div className="flex-1 min-w-0">
                <h3
                  className={`font-semibold truncate ${windowWidth < 1500 ? 'text-sm' : ''}`}
                  style={{ color: colors.primary }}
                >
                  {userProfile.user.name}
                </h3>
                <p
                  className={`truncate mt-1 ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}
                  style={{ color: colors.secondary }}
                >
                  {windowWidth < 1500
                    ? (userProfile.headline?.substring(0, 40) || 'No headline')
                    : (userProfile.headline?.substring(0, 50) || 'No headline set')}
                  {userProfile.headline && ((windowWidth < 1500 && userProfile.headline.length > 40) || (windowWidth >= 1500 && userProfile.headline.length > 50)) ? '...' : ''}
                </p>

                {/* Stats - HORIZONTAL */}
                <div className={`flex ${windowWidth < 1500 ? 'gap-3 mt-2' : 'gap-4 mt-3'}`}>
                  <div className="text-center">
                    <div
                      className={`font-semibold ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}
                      style={{ color: colors.primary }}
                    >
                      {userProfile.socialStats?.followerCount || 0}
                    </div>
                    <div
                      className={windowWidth < 1500 ? 'text-[10px]' : 'text-xs'}
                      style={{ color: colors.secondary }}
                    >
                      Followers
                    </div>
        ...
        {ads.length > 0 && (
          ...
          >
            {/* Ad Header */}
            <div
              className={`${windowWidth < 1500 ? 'px-3 py-2' : 'px-4 py-3'} border-b flex items-center justify-between`}
              style={{
                borderColor: colors.primary + '20',
            ...
            >
              <div className="flex items-center gap-2">
                <Sparkles className={`${windowWidth < 1500 ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} style={{ color: colors.accent }} />
                <span
                  className={`font-semibold ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}
                  style={{ color: colors.primary }}
                >
              ...
              <span
                className={`${windowWidth < 1500 ? 'text-[10px] px-1.5 py-0.5' : 'text-xs px-2 py-1'} rounded-full`}
                style={{
            ...
            {/* Ads Content */}
            <div className={`${windowWidth < 1500 ? 'p-3 space-y-4' : 'p-4 space-y-5'}`}>
              {ads.map((ad, index) => (
                <div
                  key={ad.id}
                  className="animate-in fade-in-up duration-500"
        ...
        {/* Quick Stats Card */}
        <div
          className={`rounded-xl animate-in slide-in-from-right-0 duration-500 animate-delay-500 ${windowWidth < 1500 ? 'p-3' : 'p-4'}`}
          style={{
            ...getCardStyle(),
            border: `1px solid ${colors.cardBorderLight || '#e5e7eb'}`
          }}
        >
          <h4
            className={`font-semibold mb-3 flex items-center gap-2 ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}
            style={{ color: colors.primary }}
          >
            <TrendingUp className={`${windowWidth < 1500 ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
            Ad Performance
          </h4>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className={windowWidth < 1500 ? 'text-xs' : 'text-sm'} style={{ color: colors.secondary }}>Impressions</span>
              <span style={{ color: colors.primary }} className={`font-semibold ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}>
                {adStats.totalImpressions.toLocaleString()}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className={windowWidth < 1500 ? 'text-xs' : 'text-sm'} style={{ color: colors.secondary }}>Clicks</span>
              <span style={{ color: colors.primary }} className={`font-semibold ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}>
                {adStats.totalClicks.toLocaleString()}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className={windowWidth < 1500 ? 'text-xs' : 'text-sm'} style={{ color: colors.secondary }}>CTR</span>
              <span style={{ color: colors.primary }} className={`font-semibold ${windowWidth < 1500 ? 'text-xs' : 'text-sm'}`}>
          ...
          {/* View All Button */}
          <button
            className={`w-full mt-3 rounded-lg font-medium transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${windowWidth < 1500 ? 'py-2 text-xs' : 'py-2.5 text-sm'}`}
            style={{
              background: `${colors.primary}10`,
              color: colors.primary
            }}
