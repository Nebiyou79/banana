...
// components/social/post/PostCard.tsx - MOBILE OPTIMIZED PREMIUM VERSION WITH NULL SAFETY
...
// Define a safe author fallback type
...
export const PostCard: React.FC<PostCardProps> = ({
  ...
  condensed = false,
  className = '',
  isFollowing: parentIsFollowing = false,
  onFollowChange
}) => {
  const [localPost, setLocalPost] = useState<Post>(post);
  const [showComments, setShowComments] = useState(false);
  const [showMoreMenu, setShowMoreMenu] = useState(false);
  const [isFollowing, setIsFollowing] = useState(parentIsFollowing);
...
// ✅ SAFE AUTHOR FALLBACK - CRITICAL FIX
const safeAuthor = localPost.author ? {
  _id: localPost.author._id || 'deleted-user',
  name: localPost.author.name || 'Deleted User',
  avatar: localPost.author.avatar || null,
  verificationStatus: localPost.author.verificationStatus || 'none',
  role: localPost.author.role || 'user',
  company: localPost.author.company || '',
  headline: localPost.author.headline || ''
} : {
  _id: 'deleted-user',
  name: 'Deleted User',
  avatar: null,
  ...
  // ✅ Check if post has valid author (for hiding invalid posts)
  const hasValidAuthor = localPost.author !== null && localPost.author !== undefined;
  
  // If post has no author, show minimal error state
  if (!hasValidAuthor) {
    return (
      <article className={`w-full bg-white dark:bg-gray-900 ${isMobile ? 'border-0' : 'border border-gray-200/60 dark:border-gray-800/60 rounded-2xl shadow-lg'} transition-all duration-300 ${className}`}>
  ...
  // ✅ Use safeAuthor for all subsequent checks
  const isOwnPost = safeAuthor._id === currentUserId;
  ...
  // Follow handler - ✅ Using safeAuthor._id
  const handleFollow = async () => {
    ...
    // Notify parent of the change - ✅ Using safeAuthor._id
    if (onFollowChange) {
      onFollowChange(safeAuthor._id, !previousFollowing);
    }

    try {
      const result = await followService.toggleFollow(safeAuthor._id);

      // Update with actual result
      setIsFollowing(result.following);
      if (onFollowChange) {
        onFollowChange(safeAuthor._id, result.following);
      }
      ...
      toast({
        ...
        description: result.following
          ? `You are now following ${safeAuthor.name}`
          : `You unfollowed ${safeAuthor.name}`,
        duration: 2000
      });
    } catch (error: any) {
      // Rollback on error
  ...
  // ✅ Get author badge - Using safeAuthor
  const getAuthorBadge = () => {
    const { verificationStatus, role } = safeAuthor;
    ...
    if (verificationStatus === 'verified') {
      return {
        ...
        label: "Verified",
        className: "bg-linear-to-r from-blue-500 to-cyan-400 text-white",
      };
    }
      ...
      <div className={isMobile ? 'px-0' : 'px-6'}>
        ...
        <div className={`${isMobile ? 'px-4 pt-4' : 'pt-6'} flex items-start justify-between`}>
          ...
          <div className="flex items-start gap-3 flex-1">
            {/* Avatar - ✅ Using safeAuthor */}
            <button
              onClick={() => {
                if (onViewProfile) {
                  onViewProfile(safeAuthor._id);
                } else {
                  window.location.href = `/social/profile/${safeAuthor._id}`;
                }
              }}
              className="shrink-0"
              aria-label={`View ${safeAuthor.name}'s profile`}
            >
              <Avatar className={`${isMobile ? 'w-10 h-10' : 'w-12 h-12'} ring-2 ring-gray-100 dark:ring-gray-800`}>
                {safeAuthor.avatar ? (
                  ...
                  <AvatarFallback className={`${isMobile ? 'text-sm' : 'text-base'} bg-linear-to-br from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 text-blue-600 dark:text-blue-400 font-medium`}>
                    {profileService.getInitials(safeAuthor.name)}
                  </AvatarFallback>
                )}
              </Avatar>
            </button>
            ...
            <div className="flex-1 min-w-0">
              {/* Name and Badge Row - ✅ Using safeAuthor */}
              ...
              <div className="flex items-center flex-wrap gap-1.5">
                ...
                {/* Optional Metadata Toggle - ✅ Using safeAuthor */}
                {(safeAuthor.company || safeAuthor.headline || post.location?.name) && (
                  <>
                    <span className="text-gray-400">•</span>
                    <button
              ...
              {/* Expandable Metadata - ✅ Using safeAuthor */}
              {showFullMetadata && (
                <div className="mt-2 space-y-1">
                  {safeAuthor.company && (
                    <div className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300">
                      <Building className="w-3 h-3" />
                      <span className="font-medium">{safeAuthor.company}</span>
                    </div>
                  )}
                  {safeAuthor.headline && (
        ...
        <div className={`${isMobile ? 'px-4 py-3' : 'py-4'}`}>
          <PostActions
            ...
            onShare={handleShare}
            onSave={handleSave}
            onInteractionUpdate={handlePostUpdate}
          />
        </div>
