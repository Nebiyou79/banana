/**
 * screens/company/MoreScreen.tsx
 *
 * Company "More" hub — surfaces every screen not in the 6 main tabs:
 *   • Freelancer Marketplace (Shortlist)
 *   • Verification + Request Verification
 *   • Referrals & Rewards / Leaderboard
 *   • Public Product Marketplace
 *   • Company Products (own product management)
...
const ACCENT = '#3B82F6'; // Company accent — blue
...
export const CompanyMoreScreen: React.FC = () => {
  ...
  const initials   = (user?.name ?? 'C').split(' ').map((p: string) => p[0]).join('').toUpperCase().slice(0, 2);
  ...
  const freelancerSection: MenuItem[] = [
    {
      ...
      sublabel: 'Browse and hire freelancers',
      color: '#10B981',
    ...
    },
    {
      ...
      sublabel: 'Your saved freelancer list',
      color: '#10B981',
      screen: 'FreelancerShortlist',
    },
  ];

  const productsSection: MenuItem[] = [
    {
      icon: 'cube-outline',
      label: 'My Products',
  ...
  const verificationSection: MenuItem[] = [
    {
      ...
      sublabel: isVerified
        ...
        : 'Get verified to attract better talent',
  ...
  const rewardsSection: MenuItem[] = [
    {
      ...
      sublabel: 'Invite businesses, earn points',
      color: '#F59E0B',
      screen: 'Referral',
    },
    {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={[ms.rolePill, { backgroundColor: ACCENT + '18', marginTop: 4 }]}>
            ...
            <Text style={{ color: ACCENT, fontSize: typography.xs, fontWeight: '700' }}>Company</Text>
          </View>
        </View>
      ...
      <MenuSection title="Products"      items={productsSection}     navigation={navigation} />
