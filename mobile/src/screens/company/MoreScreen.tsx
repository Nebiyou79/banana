...
export const CompanyMoreScreen: React.FC = () => {
  ...
  const initials  = (user?.name??'C').split(' ').map((x)=>x[0]).join('').toUpperCase().slice(0,2);
  const vStatus   = verification?.verificationStatus ?? 'none';
  const items = [
    { icon:'briefcase-outline', label:'Job Postings', color:C_ACCENT },
    { icon:'people-outline',    label:'Applicants',   color:C_ACCENT },
    { icon:'shield-checkmark-outline', label:'Verification', sublabel:vStatus==='full'?'Verified ✓':vStatus==='partial'?'Partially verified':'Get verified', color:vStatus==='full'?'#10B981':C_ACCENT, screen:'VerificationStatus' as const },
    ...
    <ScrollView style={{flex:1,backgroundColor:colors.background}} contentContainerStyle={{padding:spacing[5],paddingTop:56}} showsVerticalScrollIndicator={false}>
      <View style={[cs.userCard,{backgroundColor:colors.surface,borderColor:colors.border}]}>
        {avatarUrl?<Image source={{uri:avatarUrl}} style={cs.moreAvatar}/>:
          <View style={[cs.moreAvatar,{backgroundColor:C_ACCENT,alignItems:'center',justifyContent:'center'}]}><Text style={{color:'#fff',fontWeight:'700',fontSize:typography.lg}}>{initials}</Text></View>}
        <View style={{flex:1}}>
          ...
          <View style={[cs.roleBadge,{backgroundColor:C_ACCENT+'18'}]}><Text style={{color:C_ACCENT,fontSize:typography.xs,fontWeight:'600'}}>Company</Text></View>
        </View>
      </View>
      ...
      <Text style={{color:colors.textMuted,fontSize:typography.xs,textAlign:'center',marginTop:16}}>Banana v1.0.0 · Company</Text>
    </ScrollView>
  );
};
// ─── Styles ───────────────────────────────────────────────────────────────────
