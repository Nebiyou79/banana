...
const StatTile: React.FC<{ label:string; value:number; icon:string; color:string }> = ({ label, value, icon, color }) => {
  ...
  return (
    <View style={[cs.statCard,{backgroundColor:colors.surface,borderColor:colors.border}]}>
      <View style={[cs.statIcon,{backgroundColor:color+'18'}]}><Ionicons name={icon as any} size={20} color={color}/></View>
      <Text style={[cs.statVal,{color:colors.text}]}>{value}</Text>
      <Text style={[cs.statLbl,{color:colors.textMuted}]}>{label}</Text>
    </View>
  );
};
...
export const CompanyDashboardScreen: React.FC = () => {
  ...
  const { data: stats, isLoading } = useQuery({ queryKey:['company','stats'], queryFn: companyService.getDashboardStats, staleTime:5*60*1000 });
  const { data: jobs } = useQuery({ queryKey:['company','jobs'], queryFn: companyService.getMyJobs, staleTime:5*60*1000 });
  ...
  return (
    <ScrollView style={{flex:1,backgroundColor:colors.background}} contentContainerStyle={{padding:spacing[5],paddingTop:56}} showsVerticalScrollIndicator={false}>
      ...
      <View style={[cs.card,{backgroundColor:colors.surface,borderColor:colors.border}]}>
        <View style={cs.row}><Text style={{color:colors.text,fontWeight:'600'}}>Company profile</Text><Text style={{color:C_ACCENT,fontWeight:'700'}}>{completion}%</Text></View>
        <View style={[cs.barBg,{backgroundColor:colors.border}]}><View style={[cs.barFill,{width:`${completion}%` as any,backgroundColor:C_ACCENT}]}/></View>
      </View>
      <Text style={{color:colors.text,fontWeight:'700',marginBottom:12,fontSize:typography.base}}>Overview</Text>
      {isLoading?<ActivityIndicator color={C_ACCENT} style={{marginVertical:20}}/>:(
        <View style={cs.statsRow}>
          <StatTile label="Jobs"         value={stats?.totalJobs??0}         icon="briefcase"      color={C_ACCENT}/>
      ...
      {(jobs?.length??0)>0&&(
        <>
          <Text style={{color:colors.text,fontWeight:'700',marginBottom:12,fontSize:typography.base,marginTop:20}}>Recent Jobs</Text>
          {jobs!.slice(0,4).map((job)=>(
            <View key={job._id} style={[cs.jobRow,{backgroundColor:colors.surface,borderColor:colors.border}]}>
              <View style={{flex:1}}>
                <Text style={{color:colors.text,fontWeight:'600',fontSize:typography.sm}}>{job.title}</Text>
                <Text style={{color:colors.textMuted,fontSize:typography.xs}}>{job.location??''} · {job.jobType??''}</Text>
              </View>
