/**
 * mobile/src/screens/company/CompanyProductDetailsScreen.tsx
 *
 * UPDATED:
 *  - useIsCompanyOwner(product) replaces 12 lines of inline ownership math
 *  - useCompanyId() replaces inline user.company resolution
 *  - All hardcoded hex colors removed:
 *      '#FBBF24'  → colors.warning
 *      '#22C55E'  → colors.success
 *      '#fff'     → colors.textInverse
 *  - Stat sub-component now uses AppColors type, not Record<string, string>
 *  - Related cards always render with explicit `variant` discriminator
 *  - Pull-to-refresh added to the ScrollView
 */
import React, { useState, useCallback } from 'react';
import {
  View, Text, ScrollView, TouchableOpacity, StyleSheet,
  SafeAreaView, ActivityIndicator, StatusBar,
  Alert, ActionSheetIOS, Platform, RefreshControl,
} from 'react-native';
...
import {
  ...
  useDeleteProduct, useUpdateProductStatus,
...
import {
  formatPrice, getStockStatus, getStockBadgeConfig, getProductStatusConfig,
...
export const CompanyProductDetailsScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleOwnerActions = useCallback(() => {
    ...
    if (Platform.OS === 'ios') {
      ...
      Alert.alert('Product Actions', 'Choose an action', [
        ...
        { text: isOOS ? 'Set as Active' : 'Mark Out of Stock',
          onPress: () => updateStatus({ id: productId, status: (isOOS ? 'active' : 'out_of_stock') as ProductStatus }) },
        { text: 'Delete Product', style: 'destructive',
  ...
  // Map publish toggle colors via the design system tokens
  const publishOn  = product.status === 'active';
  const publishCol = publishOn ? colors.error   : colors.success;
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: colors.bgPrimary }]}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingBottom: isOwner ? 120 : 32 }}
        refreshControl={
        ...
        <View style={{ padding: 16, gap: 14 }}>
          ...
          <View style={s.priceRow}>
            ...
            <View style={[s.stockBadge, { backgroundColor: stockCfg.background }]}>
              ...
              <Text style={[s.stockTxt, { color: stockCfg.color }]}>
                {stockCfg.label}
              </Text>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8 }}>
            ...
            {product.featured && (
              <View style={[s.featuredBadge, { backgroundColor: colors.warningBg }]}>
          ...
          <View style={[s.statsRow, { backgroundColor: colors.bgSurface, borderColor: colors.borderPrimary }]}>
            ...
            <Stat
              icon="cube-outline"
              label="In Stock"
              value={product.inventory?.trackQuantity ? String(product.inventory.quantity) : '∞'}
              colors={colors}
            />
            <View style={[s.statDivider, { backgroundColor: colors.borderPrimary }]} />
          ...
          {product.description && (
            <View>
              ...
              <Text
                style={[s.bodyTxt, { color: colors.textSecondary }]}
              ...
              {product.description.length > 180 && (
                <TouchableOpacity onPress={() => setDescExpanded(v => !v)}>
                  <Text style={[s.showMore, { color: colors.accent }]}>
          ...
          {!!product.tags?.length && (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
              {product.tags.map((tag, i) => (
                <View
                  key={`tag-${tag}-${i}`}
                  style={[s.tag, { backgroundColor: colors.inputBg, borderColor: colors.borderPrimary }]}
                >
          ...
          {/* Related — plain ScrollView avoids nested-VirtualizedList warning */}
          {related.length > 0 && (
            <View>
                  ...
                  if (relatedIsOwn) {
                    return (
                      <CompanyProductCard
                        ...
                        product={item}
                        onPress={() =>
                          navigation.replace('CompanyProductDetails', { productId: item._id })
                        }
                        onEdit={() =>
      ...
      {isOwner && (
        <View style={[s.bottomBar, { backgroundColor: colors.bgSurface, borderTopColor: colors.borderPrimary }]}>
          ...
          <TouchableOpacity
            onPress={() => updateStatus({ id: productId, status: publishOn ? 'draft' : 'active' })}
            disabled={statusLoading}
            style={[s.actionBtn, { borderColor: publishCol }]}
            activeOpacity={0.85}
          >
            {statusLoading ? (
              ...
              <>
                <Ionicons
                  name={publishOn ? 'eye-off-outline' : 'eye-outline'}
                  size={18}
                  color={publishCol}
                />
                <Text style={{ fontSize: 13, fontWeight: '700', color: publishCol }}>
                  {publishOn ? 'Unpublish' : 'Publish'}
                </Text>
              </>
            )}
          </TouchableOpacity>
...
const Stat: React.FC<StatProps> = ({ icon, label, value, colors }) => (
...
const s = StyleSheet.create({
  ...
  navbar:       {
    ...
    paddingHorizontal: 4, paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth,
  },
  ...
  notFoundTxt:  { fontSize: 17, fontWeight: '600', textAlign: 'center' },
  statusBanner: {
  ...
  bottomBar:    {
    ...
    paddingBottom: 24, borderTopWidth: StyleSheet.hairlineWidth, gap: 8,
    position: 'absolute', bottom: 0, left: 0, right: 0,
  },
  actionBtn:    {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    paddingVertical: 12, gap: 5, borderWidth: 1.5, borderRadius: 12,
  },
  deleteBtn:    {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    paddingVertical: 12, gap: 5, borderRadius: 12,
  },
});