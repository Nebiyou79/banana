...
// Hero capped at 35% viewport height
const HERO_HEIGHT = Math.min(280, height * 0.35);
...
export const PortfolioDetailsScreen: React.FC = () => {
  ...
  const imageUrls = (item?.mediaUrls ?? []).filter((u: string) => u?.includes('cloudinary.com'));
  ...
  return (
    <ScreenWrapper>
      ...
      >
        {/* Image carousel — height capped at 35vh */}
        {imageUrls.length > 0 && (
          <View>
            <Image
              source={{ uri: getOptimizedUrl(imageUrls[activeImage], 800, 500) }}
              style={[styles.heroImage, { width, height: HERO_HEIGHT }]}
              resizeMode="cover"
            />
            {imageUrls.length > 1 && (
              <>
                <ScrollView
                  ...
                  style={[styles.thumbnailScroll, { backgroundColor: colors.bgCard }]}
                >
        ...
        <View style={{ padding: spacing.xl }}>
          ...
          <View style={styles.titleRow}>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: FONT_SIZE.xxl, fontWeight: '800', color: colors.text }}>
                {item.title}
              </Text>
              <View style={styles.metaBadges}>
                {item.featured && (
                  <View style={[styles.metaBadge, { backgroundColor: withAlpha(colors.warning, 0.12), borderRadius: 10 }]}>
                    <Ionicons name="star" size={11} color={colors.warning} />
                    <Text style={{ fontSize: 10, color: colors.warning, fontWeight: '700', marginLeft: 3 }}>Featured</Text>
                  </View>
                )}
                {item.visibility && (
                  <View style={[styles.metaBadge, {
                    backgroundColor: item.visibility === 'public' ? colors.successBg : colors.border,
                    borderRadius: 10,
                  }]}>
                    <Ionicons
          ...
          <View style={[styles.statsGrid, {
            ...
            {item.budget != null && (
              <View style={[styles.statCell, { borderRightWidth: 1, borderRightColor: colors.border }]}>
                ...
                <Text style={{ fontSize: FONT_SIZE.base, fontWeight: '800', color: colors.primary, marginTop: 4 }}>
            ...
            {item.duration && (
              <View style={[styles.statCell, { borderRightWidth: item.client ? 1 : 0, borderRightColor: colors.border }]}>
                ...
                <Text style={{ fontSize: FONT_SIZE.base, fontWeight: '800', color: colors.text, marginTop: 4 }}>
                  {item.duration}
                </Text>
            ...
            {item.client && (
              <View style={styles.statCell}>
                ...
                <Text style={{ fontSize: FONT_SIZE.sm, fontWeight: '700', color: colors.text, marginTop: 4 }} numberOfLines={1}>
                  {item.client}
                </Text>
          ...
          {item.description && (
            <View style={{ marginTop: spacing.xl }}>
              ...
              <Text style={{ fontSize: FONT_SIZE.base, color: colors.textSecondary, lineHeight: 24 }}>
                {item.description}
              </Text>
            </View>
          )}
              ...
              <View style={styles.tagWrap}>
                {item.technologies.map((t: string, i: number) => (
                  <View key={i} style={[styles.techTag, {
                    ...
                    <Text style={{ fontSize: FONT_SIZE.xs, color: colors.primary, fontWeight: '600' }}>{t}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          {item.projectUrl && (
            ...
            >
              ...
              <Text style={{ fontSize: FONT_SIZE.sm, color: colors.primary, fontWeight: '700', marginLeft: 8 }}>
                View Live Project
              </Text>
          ...
          <View style={[styles.actionsRow, { marginTop: spacing.xl }]}>
            <TouchableOpacity
              onPress={() => navigation.navigate('EditPortfolio', { itemId: item._id })}
              style={[styles.actionBtn, { backgroundColor: colors.primary, borderRadius: radius.lg, flex: 1 }]}
            >
              <Ionicons name="pencil-outline" size={16} color="#fff" />
              <Text style={{ color: '#fff', fontWeight: '700', fontSize: FONT_SIZE.sm, marginLeft: 6 }}>
                Edit Project
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={handleDelete}
              style={[styles.actionBtn, { backgroundColor: colors.dangerBg, borderRadius: radius.lg, width: 50 }]}
            >
...
const SectionTitle: React.FC<{ icon: keyof typeof Ionicons.glyphMap; label: string }> = ({ icon, label }) => {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
      ...
      <Text style={{ fontSize: FONT_SIZE.base, fontWeight: '700', color: colors.text, marginLeft: 8 }}>
        {label}
      </Text>
    </View>
  );
