 ...
 * Refactored: useTheme() only, colors.candidate for role accent,
 * colors.bg/bgCard, no hardcoded hex, initials from text utils.
 */
...
const ProfileHeroCard = React.memo<HeroCardProps>(
  ({ name, email, avatarUrl, onEdit, colors: c }) => (
    <View style={[hero.card, { backgroundColor: c.bgCard, borderColor: c.border }]}>
      ...
      <TouchableOpacity
        onPress={onEdit}
        style={[hero.editBtn, { backgroundColor: c.bg }]}
...
const hero = StyleSheet.create({
  ...
  onlineDot: {
    ...
    width: 14, height: 14, borderRadius: 7, backgroundColor: '#10B981', borderWidth: 2,
  },
...
const ProfileStatChip: React.FC<{
  ...
  <View style={[psc.chip, { backgroundColor: c.bgCard, borderColor: c.border }]}>
...
const psc = StyleSheet.create({
  chip:    { flex: 1, minWidth: 90, alignItems: 'center', borderRadius: 14, borderWidth: 1, paddingVertical: 12, paddingHorizontal: 6, gap: 4 },
...
const ExperienceRow: React.FC<ExpRowProps> = React.memo(
  ({ position, company, range, current, colors: c }) => (
    <View style={[er.row, { backgroundColor: c.bgCard, borderColor: c.border }]}>
      <View style={[er.dot, { backgroundColor: current ? c.success : c.candidate }]} />
      <View style={{ flex: 1 }}>
        <Text style={{ color: c.text, fontWeight: '700', fontSize: 13 }}>{position}</Text>
        <Text style={{ color: c.candidate, fontWeight: '600', fontSize: 12 }}>{company}</Text>
        <Text style={{ color: c.textMuted, fontSize: 11, marginTop: 2 }}>{range}</Text>
      </View>
      {current && (
        <View style={[er.currentPill, { backgroundColor: c.successBg }]}>
          <Text style={[er.currentText, { color: c.success }]}>Current</Text>
        </View>
      )}
    </View>
  ),
...
const er = StyleSheet.create({
  ...
  currentPill: { borderRadius: 99, paddingHorizontal: 8, paddingVertical: 3 },
...
export const CandidateDashboardScreen: React.FC = () => {
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={{ paddingTop: 56, paddingBottom: 48, paddingHorizontal: spacing.lg }}
    ...
    >
      ...
      <View style={s.profileStatsGrid}>
        <ProfileStatChip icon="school-outline"        label="Education"  value={education.length}  color={colors.info}      colors={colors} />
        <ProfileStatChip icon="briefcase-outline"     label="Experience" value={experience.length} color={colors.candidate} colors={colors} />
        <ProfileStatChip icon="flash-outline"         label="Skills"     value={skills.length}     color={colors.success}   colors={colors} />
      </View>
      <View style={[s.profileStatsGrid, { marginTop: 10 }]}>
        <ProfileStatChip icon="ribbon-outline"        label="Certs"    value={certs.length}     color={colors.freelancer} colors={colors} />
        <ProfileStatChip icon="person-circle-outline" label="Strength" value={`${completion}%`} color={colors.danger}     colors={colors} />
      </View>

      {/* ── 4. Profile strength bar ─────────────────────────── */}
      <View style={[s.strengthCard, { backgroundColor: colors.bgCard, borderColor: colors.border }]}>
        <View style={s.row}>
          <Text style={{ color: colors.text, fontWeight: '600', fontSize: 14 }}>
            Profile strength
          </Text>
          <Text style={{ color: colors.candidate, fontWeight: '800', fontSize: 14 }}>{completion}%</Text>
        </View>
        <View style={[s.barBg, { backgroundColor: colors.border }]}>
          <View
            style={[
              s.barFill,
              { width: `${completion}%` as any, backgroundColor: colors.candidate },
            ]}
          />
        </View>
      ...
      {statsLoading ? (
        ...
        <View style={s.statsGrid}>
          <StatCard label="Total"       value={stats?.total ?? 0}       icon="document-text-outline" color={colors.candidate} />
          <StatCard label="Pending"     value={stats?.pending ?? 0}     icon="time-outline"          color={colors.info}      />
          <StatCard label="Shortlisted" value={stats?.shortlisted ?? 0} icon="star-outline"          color={colors.success}   />
          <StatCard label="Rejected"    value={stats?.rejected ?? 0}    icon="close-circle-outline"  color={colors.danger}    />
        </View>
      )}
        ...
        <>
          ...
          <View style={s.skillsWrap}>
            {skills.slice(0, 12).map((sk: string) => (
              <View
                key={sk}
                style={[s.skillChip, { backgroundColor: colors.candidate + '18', borderColor: colors.candidate + '40' }]}
              >
...
const s = StyleSheet.create({
  ...
  skillChip:        { paddingHorizontal: 12, paddingVertical: 5, borderRadius: 99, borderWidth: 1 },
});