/**
 * src/screens/organization/OrgJobDetail.tsx
 * Org-side employer detail screen — mirrors CompanyJobDetailScreen.
 */
...
import { useJob, useDeleteOrganizationJob, useUpdateOrganizationJob } from '../../hooks/useJobs';
...
export const OrgJobDetail: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleStatusToggle = useCallback(() => {
    ...
    updateMut.mutate({ id: jobId, data: { status: next } });
  }, [job, jobId, updateMut]);

  if (jobQ.isLoading) {
    return (
      <SafeAreaView style={[s.root, { backgroundColor: c.background }]} edges={[]}>
        <View style={[s.loadingHeader, { backgroundColor: '#1E1142' }]}>
    ...
    return (
      <SafeAreaView style={[s.root, { backgroundColor: c.background }]} edges={['top']}>
        <View style={s.center}>
          ...
          <Text style={[s.notFound, { color: c.text }]}>Opportunity not found</Text>
    ...
    <SafeAreaView style={[s.root, { backgroundColor: c.background }]} edges={[]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
        ...
        <View style={[s.actions, { backgroundColor: c.surface, borderBottomColor: c.border }]}>
          <ActionBtn icon="pencil-outline" label="Edit" color={c.primary}
            onPress={() => navigation.navigate('OrgJobEdit', { jobId })} />
          <View style={[s.div, { backgroundColor: c.border }]} />
          <ActionBtn icon="people-outline" label={`${applicantCount}`} color="#8B5CF6"
          ...
          />
          <View style={[s.div, { backgroundColor: c.border }]} />
          <ActionBtn icon="trash-outline" label="Delete" color={c.error}
            onPress={handleDelete} loading={deleteMut.isPending} />
        </View>

        {/* Stats */}
        <View style={[s.statsCard, { backgroundColor: c.surface, borderColor: c.border }]}>
          {[
            { value: applicantCount, label: 'Applicants', icon: 'people-outline', color: '#3B82F6' },
            { value: job.candidatesNeeded ?? 1, label: 'Positions', icon: 'person-outline', color: '#10B981' },
            { value: job.viewCount ?? 0, label: 'Views', icon: 'eye-outline', color: '#F59E0B' },
            { value: job.saveCount ?? 0, label: 'Saves', icon: 'bookmark-outline', color: '#8B5CF6' },
          ].map((stat, i) => (
          ...
          <Section title="Requirements" icon="checkmark-circle-outline" c={c}>
            {job.requirements!.map((r, i) => (
              <View key={i} style={s.bulletRow}>
                <Ionicons name="checkmark-circle" size={15} color={c.primary} style={{ marginTop: 2 }} />
                <Text style={[s.bulletText, { color: c.textSecondary ?? c.textMuted }]}>{r}</Text>
              </View>
            ))}
          </Section>
        )}
        ...
        {(job.skills ?? []).length > 0 && (
          <Section title="Required Skills" icon="sparkles-outline" c={c}>
            <View style={s.tagsRow}>
              {job.skills!.map((sk, i) => (
                <View key={i} style={[s.tag, { backgroundColor: '#7C3AED18', borderColor: '#7C3AED30' }]}>
                  <Text style={[s.tagText, { color: '#7C3AED' }]}>{sk}</Text>
                </View>
              ))}
            </View>
          </Section>
...
const ActionBtn = ({ icon, label, color, onPress, loading }: any) => (
...
const s = StyleSheet.create({
  ...
  div:           { width: 1, height: '60%', alignSelf: 'center' },
