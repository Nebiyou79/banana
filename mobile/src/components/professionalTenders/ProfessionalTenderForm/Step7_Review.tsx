// ─────────────────────────────────────────────────────────────────────────────
//  src/components/professionalTenders/ProfessionalTenderForm/Step7_Review.tsx
// ─────────────────────────────────────────────────────────────────────────────
//  Step 7: Review.  Read-only summary of every step. The shell renders the
//  Save Draft / Publish buttons in its footer — this step just shows what
//  the user is about to submit.
//
//  P-14: preBidMeeting block uses ROOT-LEVEL values via `useWatch('preBidMeeting')`.
//        Never reach into procurement.
// ─────────────────────────────────────────────────────────────────────────────
...
import type { StagedFile } from './Step6_Documents';
...
export interface Step7_ReviewProps {
  files: StagedFile[];
}

const Step7_Review: React.FC<Step7_ReviewProps> = ({ files }) => {
  const isDark = useThemeStore((s) => s.theme.isDark);
  const { control, formState: { isValid, errors } } = useFormContext<ProfessionalTenderFormValues>();

  // Watch every field so the review reflects the latest values without
  // re-rendering the entire form on every keystroke (RHF handles this).
  const values = useWatch({ control }) as Partial<ProfessionalTenderFormValues>;

  const errorCount = useMemo(() => {
    // Count leaf errors — gives a useful "X issues remain" message
    let n = 0;
    const visit = (e: any) => {
      if (!e) return;
    ...
    <View style={styles.root}>
      ...
      {isValid && errorCount === 0 ? (
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.statusTitle, { color: palette.statusOkFg }]}>
              Ready to submit
            </Text>
      ...
      >
        <Row label="Min. Experience" value={elig?.minimumExperience !== undefined && elig?.minimumExperience !== null ? `${elig.minimumExperience} years` : ''} />
        <Row
      ...
      >
        <Row
          label="Method"
          value={evaln?.evaluationMethod ? EVAL_METHOD_LABELS[evaln.evaluationMethod] : ''}
        />
      ...
      {/* ─── 7. CPO requirement (top-level toggle) ────────────────────── */}
      <SummaryCard
        icon={<Award size={16} color={palette.iconColor} strokeWidth={2.4} />}
        title="CPO"
      >
        <Row label="CPO Required" value={values.cpoRequired ? 'Yes' : 'No'} />
        {!!values.cpoDescription && (
          <View style={styles.scopeBlock}>
            <Text style={[styles.subHead, { color: palette.muted }]}>NOTES</Text>
            <Text style={[styles.scopeText, { color: palette.text }]}>{values.cpoDescription}</Text>
          </View>
        )}
      </SummaryCard>

      {/* Footnote — what happens when published */}
