 ...
 * Unified avatar + cover uploader for all four role profile screens.
 * Mirrors web AvatarUploader.tsx behavior exactly.
 *
 * API contracts:
 *   POST /profile/avatar  — field: 'avatar'  → { data: { avatar: CloudinaryImage, thumbnailUrl } }
 *   POST /profile/cover   — field: 'cover'   → { data: { cover:  CloudinaryImage, thumbnailUrl } }
...
export interface ProfileImageUploaderProps {
  ...
  onCoverDeleted?: () => void;
  /** 'both' shows cover banner + avatar, 'avatar' shows only avatar, 'cover' shows only cover */
  type?: 'avatar' | 'cover' | 'both';
  /** 'circle' for candidate/freelancer, 'square' for company/org */
...
function resolveErrorMessage(err: unknown, type: 'avatar' | 'cover'): string {
  ...
  if (err.message.includes('FILE_TOO_LARGE')) {
    return type === 'avatar' ? 'Image must be under 5 MB.' : 'Cover must be under 10 MB.';
  }
...
function useImageUploader(
  ...
  // Sync when parent props update (e.g. after invalidateQueries refetch)
  // Only update if not currently uploading to avoid flicker
  useEffect(() => {
    setState(s =>
      s.uploadingAvatar ? s : { ...s, localAvatarUri: initialAvatarUrl ?? null }
    );
  }, [initialAvatarUrl]);

  useEffect(() => {
    setState(s =>
      s.uploadingCover ? s : { ...s, localCoverUri: initialCoverUrl ?? null }
    );
  }, [initialCoverUrl]);
  ...
  const pickAndUploadAvatar = useCallback(async () => {
    ...
    try {
      ...
      // Cast required: React Native FormData accepts { uri, name, type } but
      // TypeScript's web FormData types expect Blob. The cast is intentional.
...
export const ProfileImageUploader: React.FC<ProfileImageUploaderProps> = ({
  ...
  const {
    ...
    deleteAvatar,
    deleteCover,
  } = useImageUploader(currentAvatarUrl, currentCoverUrl, accentColor, {
    onAvatarUploaded,
    onCoverUploaded,
    onAvatarDeleted,
    onCoverDeleted,
  ...
  const avatarRadius = avatarShape === 'circle' ? AVATAR_SIZE / 2 : 14;
    ...
    <View style={s.root}>
        ...
        <View style={[s.coverWrap, { backgroundColor: accentColor + '28' }]}>
          {localCoverUri ? (
            ...
            <View style={[StyleSheet.absoluteFillObject as object, s.coverPlaceholder]}>
              <Ionicons name="image-outline" size={30} color={accentColor + 'AA'} />
              <Text style={[s.coverPlaceholderText, { color: accentColor + 'AA' }]}>
          ...
          {uploadingCover && (
            <View style={s.uploadOverlay}>
              ...
              <Text style={s.uploadingLabel}>Uploading cover…</Text>
            </View>
          )}
          ...
          {!uploadingCover && (
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <View style={[s.cameraBadge, { backgroundColor: colors.bananaLight }]}>
      ...
      {showAvatar && (
        <View style={[s.avatarRow, showCover && s.avatarOverlapCover]}>
          ...
          {/* Delete avatar text button */}
          {showDeleteButtons && !!localAvatarUri && !uploadingAvatar && (
            <TouchableOpacity
              style={[s.removeAvatarBtn, { borderColor: colors.border }]}
              onPress={deleteAvatar}
              activeOpacity={0.8}
            >
              <Ionicons name="trash-outline" size={13} color={colors.textMuted} />
              <Text style={[s.removeAvatarText, { color: colors.textMuted }]}>Remove</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
